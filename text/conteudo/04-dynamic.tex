...
observations within the same individual (which are not independent of each
other) and serially correlated errors. In our regression, we added mixed-effects
to the \verb|movie_id| variable, resulting in the \verb|glmmTMB::glmmTMB()|
\citep{} formula \verb=pop ~ t * genre * rating + (1|movie_id)=.
...
degeneracy, meaning it approaches a situation \citep{} where only one movie is
